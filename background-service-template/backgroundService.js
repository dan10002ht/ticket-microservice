...
class BackgroundService {
  ...
  startDeadLetterProcessor() {
    const processor = async () => {
      while (!this.isShuttingDown) {
        try {
          const failedJob = await this.redis.rpop(
            this.config.job.deadLetterQueue
          );
          if (failedJob) {
  ...
  async addToQueue(job) {
    ...
    // Update metrics
    this.metrics.queueLengths[job.priority] =
      (this.metrics.queueLengths[job.priority] || 0) + 1;
  }
  ...
  async processJob(job) {
    ...
    try {
      ...
      logger.debug(
        `Job ${job.id} completed successfully in ${processingTime}ms`
      );
    } catch (error) {
      // Handle job failure
  ...
  async handleJobFailure(job, error, startTime) {
    ...
    if (job.retries < job.maxRetries) {
      ...
      logger.error(
        `Job ${job.id} failed permanently after ${job.maxRetries} retries`
      );
